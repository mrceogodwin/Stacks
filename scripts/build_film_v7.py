#!/usr/bin/env python3
"""Stacks film v7: product voice, music bed, full-head portrait, accurate cursor."""
...
VOICE = ROOT / "voice-v7.mp3"
MUSIC = ROOT / "music-v7.wav"
MIX = ROOT / "mix-v7.mp3"
TS = ROOT / "voice-v7.timestamps.json"
ASS = ROOT / "captions-v7.ass"
...
def end_of(words: list[tuple[str, float, float]], needle: str, start_at: float = 0.0) -> float:
    n = [p.lower().strip(".,!?'\"") for p in needle.split()]
    toks = [(w.lower().strip(".,!?'\""), a, b) for w, a, b in words]
    for i in range(len(toks) - len(n) + 1):
        if toks[i][1] < start_at - 0.05:
...
def main() -> None:
    ...
    intro = compose_still("Hello.\nWelcome to Stacks.", "Built in Nigeria. Shipped on stacks.ng", ROOT / "stills" / "v7-intro.jpg")
    what = compose_still("Software\nthat works.", "Web apps, plugins, and tools for real businesses.", ROOT / "stills" / "v7-what.jpg")
    tools_still = compose_still("Five hundred\nplus tools.", "Writing. Images. Video. Marketing. SEO.", ROOT / "stills" / "v7-tools.jpg")
    sub = compose_still("New apps.\nFirst.", "Leave an email. Hear about it as it ships.", ROOT / "stills" / "v7-sub.jpg")
    terms = compose_still("Use it\nwith care.", "Terms and policy apply. No secrets in tools.", ROOT / "stills" / "v7-terms.jpg")
    outro = compose_still("Built in Nigeria.\nShipped on stacks.ng", "Explore the apps. Explore the tools.", ROOT / "stills" / "v7-outro.jpg")
    ...
    t_welcome = t_of(words, "Welcome to Stacks")
    ...
    t_writing = t_of(words, "Writing")
    # second "Writing" is later ("Writing tools draft") — first after "then filter"
    ...
    marks = [
        ("intro", t_welcome, t_home),
        ("what", t_home, t_explore),
    ...
    builders = {
        ...
        "what": lambda d, s: kenburns(what, d, s),
        "home-explore": lambda d, s: cursor_shot(home, d, s, *px(980, 280), *explore),
        ...
        "more-s": lambda d, s: kenburns(tools_still, d, s),
        "sub": lambda d, s: kenburns(sub, d, s),
        "reason": lambda d, s: kenburns(what, d, s),
